...
export function TicketThread({
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Header — always visible so the audit toggle / back are reachable */}
      <header className="h-14 shrink-0 px-3 sm:px-5 border-b border-border flex justify-between items-center gap-2">
        <div className="flex items-center gap-2 min-w-0">
          {onBack && (
            <button
              onClick={onBack}
              className="md:hidden -ml-1 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500"
      ...
      <div className="flex-1 flex min-h-0">
        {/* Messages + composer. Hidden on mobile while the audit panel is open. */}
        <div
          className={`flex-1 flex-col min-w-0 ${showAudit ? "hidden md:flex" : "flex"}`}
        >
          <div className="flex-1 overflow-y-auto px-3 sm:px-5 py-6 flex flex-col gap-4">
            {data.messages.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`max-w-[80%] sm:max-w-[70%] flex flex-col ${mine ? "items-end" : "items-start"}`}
                  >
                    <div
          ...
          <form
            className="p-3 sm:p-4 border-t border-border flex gap-2"
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <input
              ...
              className="flex-1 min-w-0 rounded-full border border-border bg-card px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-400"
            />
            <button
              type="submit"
              disabled={reply.trim().length === 0}
              className="shrink-0 rounded-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white px-4 sm:px-5 py-2.5 text-sm font-medium transition-colors"
            >
              Send
            </button>
          </form>
        ...
        {/* Audit panel — full width on mobile, fixed side panel on desktop. */}
        {showAudit && (
          <aside className="w-full md:w-80 shrink-0 md:border-l border-border flex flex-col bg-background/60">
            <div className="h-11 px-4 border-b border-border flex items-center justify-between">
              ...
              <button
                ...
                className="md:hidden text-xs text-slate-500 hover:text-slate-800 dark:hover:text-slate-200"
              >
                Close
              </button>
            </div>
